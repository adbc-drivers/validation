...
def pytest_generate_tests(metafunc) -> None:
    ...
    if metafunc.definition.name == "test_ingest_not_null":
        metafunc.parametrize(
            "driver",
            [pytest.param(driver, id=driver) for driver in drivers],
            scope="module",
            indirect=["driver"],
        )
        return
    ...
    for driver in drivers:
        ...
        for query in queries.queries.values():
            ...
            if not isinstance(query.query, model.IngestQuery):
                continue
            if not driver_manifest.features.statement_bulk_ingest:
                marks.append(pytest.mark.skip(reason="not implemented"))

            if (
                metafunc.definition.name != "test_ingest_create"
...
def test_ingest_create(
    conn: adbc_driver_manager.dbapi.Connection, query: Query
) -> None:
...
def test_ingest_replace(
    driver, conn: adbc_driver_manager.dbapi.Connection, query: Query
) -> None:
    subquery = query.query
    assert isinstance(subquery, model.IngestQuery)
